...
class AIDiagnosisControllerContractTest {
    ...
    void shouldUseCustomerContextRoleForSelfDiagnosis() throws Exception {
        ...
        when(aiDiagnosisService.diagnoseFault(eq("发动机异响"), eq("customer"), eq(null)))
        ...
        verify(aiDiagnosisService).diagnoseFault("发动机异响", "customer", null);
    }
    ...
    void shouldUseTechnicianContextRoleForProfessionalDiagnosis() throws Exception {
        ...
        when(aiDiagnosisService.diagnoseFault(eq("刹车偏软"), eq("technician"), eq(7L)))
        ...
        verify(aiDiagnosisService).diagnoseFault("刹车偏软", "technician", 7L);
    }
